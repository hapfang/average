...
! Version 12.4: Specially modified for single subchannel turbulence analysis, 4
!               rods are involved, user can adjust the effective probe range.
!               (Remember to recompile to apply the modifications). JUN,
!               2016/12/28
!**********************************************************************************************************
	...
	include "common.f"
        ...
        integer iRod, nRod, iDnB, iUpB, nprobPerRod
...
!       np here becomes the number of probe layers
	...
	write(*,*) 'Number of homogeneous directions: ', Nhom
...
!            Recnum = Nzp*((j-1)*np*Nhom+(i-1)*Nhom+ih - 1)+mzp
	  read(20, '(2I8, I3, 19E15.7)', REC = (j-1)*np*Nhom+1) 
     1     itime(j), jj, iphase(1,1,1), raw1(1:19)
	 else
          read(20, '(2I8, I3, 15E15.7)', REC = (j-1)*np*Nhom+1)
...
c         write(*,*) 'u = ', j, ih, raw(2, j, ih, ir)
c         if (j.eq.iphtime(2, k)) pause 
c	   end do ! ir
         end do ! ih
	  end do ! j
!	 end do ! ih
...
! single phase distance field correction:
	if (nd2.eq.14) then 
	 ...
	 if (mod(Nhom,2).eq.1) then
        ...
        do iRod=1,nRod
        do ih = nprobPerRod*(iRod-1)+iDnB, nprobPerRod*(iRod-1)+iUpB, 1
...
!        write(*,*) 'Step 11'
	    if (raw(15,j,ih+ih2,ir).gt.1.0*eps_ls.and.time1.gt.0) then	! Liquid phase
		flprev = fluct
		fluct = 0.0E0
...
!	 if (Nzp.eq.7) then
!	    Mcoord = (i-1)*Nhom*Nzp + (mzp-1)*Nhom + 1 
!	 else
	    Mcoord = (i-1)*Nhom*Nzp + mzp
!            Mcoord = (i-1)*Nhom*Nzp*(isym+1) + mzp   ! modified Igor 03/15/2012
!	 end if
